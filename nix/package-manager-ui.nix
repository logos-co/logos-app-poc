...
pkgs.stdenv.mkDerivation {
  ...
  preConfigure = ''
    ...
    mkdir -p ./logos_dapps/package_manager_ui/generated_code
    ...
    if [ -d "${logosPackageManager}/include" ]; then
      ...
      cp -r "${logosPackageManager}/include"/* ./logos_dapps/package_manager_ui/generated_code/
      echo "Copied include files:"
      ls -la ./logos_dapps/package_manager_ui/generated_code/
    else
    ...
    echo "Running logos-cpp-generator (general-only) for package_manager_ui..."
    logos-cpp-generator --metadata ${src}/logos_dapps/package_manager_ui/src/metadata.json --general-only --output-dir ./logos_dapps/package_manager_ui/generated_code
  ...
  configurePhase = ''
    ...
    echo "Configuring package manager UI plugin..."
    ...
    cmake -S logos_dapps/package_manager_ui -B build \
      -GNinja \
  ...
  buildPhase = ''
    ...
    cmake --build build
    echo "Package manager UI plugin built successfully!"
  ...
  installPhase = ''
    ...
    if [ -f "build/package_manager_ui.dylib" ]; then
      ...
      echo "Error: No package_manager_ui library file found"
      exit 1
    fi
